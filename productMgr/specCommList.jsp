...
<%
  ...
  ArrayList commList =commObj.genSpecComList(Integer.valueOf(iStart).intValue(),cust_id);
     int counter=commObj.getSpecCommodityNumer(cust_id);
...
<script language="javascript">
  function chechIfo()
  {
	   if(confirm('confirm the recommendation of goods£¿')) 
	{
		return true;
	}
	else
	  ...
	  <tr>
	    <td>
		     <table width="100%" border=0 cellpadding=2 cellspacing=1 align=center bgcolor="#e7e7e7">
		        <tr>
					<td class="line1" style="background-color:#e2e2e2; color:#000000;  font-weight:bold; font-size:13px;" align=center width="13%"><bean:message key="str951"/></td>
					<td style="background-color:#e2e2e2; color:#000000;  font-weight:bold; font-size:13px;" align=center width="27%"><bean:message key="str2453"/></td>
					...
					<td  style="background-color:#e2e2e2; color:#000000;  font-weight:bold; font-size:13px;" align=center width="5%"><bean:message key="str789"/></td>
		        </tr>
		        <%
		            if(commList != null && commList.size()>0 )
		            {
		              	 for (Iterator it = commList.iterator(); it.hasNext();)
		                  {
						        ...
						        <tr style="background-color:#f9f9f9; " id="changcolor_tr<%=commodity_id%>" onmouseover="MM_changeProp('changcolor_tr<%=commodity_id%>','','backgroundColor','#ffffff','DIV')" onmouseout="MM_changeProp('changcolor_tr<%=commodity_id%>','','backgroundColor','#f9f9f9','DIV')">
					              <td  style=" color:#000000;" align=left><%=commodity_name%></td>
					              <td  style=" color:#000000;" align=left><%=commodity_brand%></td>
					              <td  style=" color:#000000;" align=left><%=commodity_type%></td>
					              <td  style=" color:#000000;" align=right><%=commodity_price%></td>
					              <td  style=" color:#000000;" align=left><%=publish_date%></td>
					              <td  style=" color:#000000;" align=left><%=sale_market%></td>
					              <td  style=" color:#000000;" align=center><a href=/doTradeReg.do?commodity_id=<%=commodity_id%>&trade_type_code=0310 target="_self"  onClick="return chechIfo()"><img src=/img/edit.gif width=16 height=16 border=0></a></td>
					            </tr>
					     ...
					     <tr>
							<td  align="left" colspan="3" class="line1" style="font-weight:normal;"><bean:message key="str1027"/><%=counter%><bean:message key="str1028"/> &nbsp;<bean:message key="str1027"/><%=pages%><bean:message key="str1029"/></td>
							<td  align="left" colspan="4"><a href="specCommIndex.jsp?iStart=0&meun_id=<%=meun_idx%>"><bean:message key="str1030"/> </a>&nbsp; &nbsp;<a href="specCommIndex.jsp?iStart=<%=pageUp%>&meun_id=<%=meun_idx%>"><bean:message key="str1031"/></a> &nbsp;<a href="specCommIndex.jsp?iStart=<%=pageDown%>&meun_id=<%=meun_idx%>"><bean:message key="str1032"/> </a>&nbsp; <a  href="specCommIndex.jsp?iStart=<%=pages%>&meun_id=<%=meun_idx%>"><bean:message key="str1033"/></a></td>
				         </tr>
