...
<%
	...
	String cust_id = "", del_id = "", con_id = "",obj_cust_id="";
	del_id = comm.GenTradeId();
	...
	font-size: 12px;
		...
		<script type="text/javascript">
		 ...
		 function confirmsub(){
		 	...
		 	document.getElementById('con_no').value=con_no;
		    if(document.getElementById("con_no").value=="" || document.getElementById("con_no").value==null){
		       alert("请选择合同编码！");
		       return false;
		     }
		    
		     if(document.getElementById("del_date").value=="" || document.getElementById("del_date").value==null){
		       alert("请选择回收时间！");
		       return false;
		     }
		     if(document.getElementById("del_reason").value=="" || document.getElementById("del_reason").value==null){
		       alert("请填写回收原因！");
		       return false;
		     }
		     return true;
				...
				<tr>
					<td valign="top">
						<table width=727 border=0 cellpadding=5 cellspacing=1 align=center bgcolor="#dddddd">
							<tr>
								...
								<td align=left bgcolor="#FFFFFF">
									<div class="ping">
										<ul style="width: 100%">
											<div class="ping1">
												<input type="checkbox" id="checkAll" name="checkAll" value="" onclick="checkAllConNo()">
												<bean:message key="str1581"/>
												<hr>
											</div>
											<%
												...
												if (conList != null && conList.size() > 0) {
													for (int k = 0; k < conList.size(); k++) {
														...
														if (map.get("con_id") != null) {
															con_id1 = map.get("con_id").toString();
														}
											%>
											<li style="width: 30%; float: left">
												<input type="checkbox" name="con<%=k%>" id="con<%=k%>" value="<%=con_no%>">
												<a href="/conbackMgr/disConuse.jsp?con_id=<%=con_id1 %>"><%=con_no %></a>
											</li>
											<%
												}
												}
							...
							<tr>
								<td style="background-color: #f6f6f6; color: #000000; font-weight: bold; font-size: 12px;" align=right>
									<bean:message key="str1582"/>
								</td>
								<td align=left bgcolor="#FFFFFF">
									<div class="ping">
										<input name="del_date" type="text" id="del_date" size=10 maxlength=15 value="<%=start_Date%>" onfocus="setday(this);">
							...
							<tr>
								<td style="background-color: #f6f6f6; color: #000000; font-weight: bold; font-size: 12px;" align=right>
									<bean:message key="str1583"/>
								</td>
								<td align=left bgcolor="#FFFFFF">
									<div class="ping">
										<textarea rows="6" cols="50" name="del_reason" id="del_reason"></textarea>
									</div>
								</td>
							</tr>
							<tr>
							...
							<tr>
								<td>
									...
									<input name="trade_type_code" type="hidden" value="3349">
									<input name="del_id" type="hidden" id="del_id" value="<%=del_id%>">
