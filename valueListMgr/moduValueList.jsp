...
<%@ page import="com.saas.biz.valueListMgr.ValueListInfo"%>
<%
    ...
    ValueListInfo valueInfo = new ValueListInfo();
    ArrayList valueList=valueInfo.getAllValueList(Integer.parseInt(iStart),cust_id);
	int counter=valueInfo.getAllValueList(cust_id);
					...
					<table width=100% border=0 cellpadding=5 cellspacing=1 align=center bgcolor="#dddddd">
						...
						<%
						 if(valueList !=null && valueList.size()>0){
						   for(int i=0;i<valueList.size();i++){
						     ...
						     String value_name="",test_id="",obj_cust_id="";
						     ...
						     if(map.get("value_name")!=null){value_name=map.get("value_name").toString();}
						     if(map.get("test_id")!=null){test_id=map.get("test_id").toString();}
						     %>
						     <tr style="background-color:#f9f9f9;">
				              <td style="color:#000000;" align="left"><%=cust_name%></td>
				              <td style="color:#000000;" align="left"><%=value_name%></td>
				              ...
				              <td style="color:#000000;" align="center"><a href="updateValueInfo.jsp?menu_id=<%=meun_idx%>&test_id=<%=test_id%>&obj_cust_id=<%=obj_cust_id%>" TARGET=appwin onclick="mydefss()"><img src=/img/edit.png width=16 height=16 border=0 style="cursor: hand" alt="ÐÞ¸Ä·µÀû"></a></td>
				            </tr>
						    <%
						   }
						   }
						%>
						 <tr>
																		...
																		<td  align="left" colspan="2" style="font-weight:normal; padding:2px 5px;"><bean:message key="str610"/><%=counter%><bean:message key="str611"/> &nbsp;<bean:message key="str759"/><%=Integer.parseInt(iStart)+1 %>Ò³<bean:message key="str612"/>&nbsp;&nbsp;<bean:message key="str610"/><%=pages%><bean:message key="str612"/></td>
											
																		<td  align="right" colspan="3"  style=" padding:2px 5px;">
																		<a href="modiValueIndex.jsp?iStart=0"><bean:message key="str1030"/></a>&nbsp; &nbsp;
																		<% 
																			if(Integer.parseInt(iStart)>0){
																		%>
																			<a href="modiValueIndex.jsp?iStart=<%=pageUp%>"><bean:message key="str1031"/></a> &nbsp;
																		<%
																			}
																			if(Integer.parseInt(iStart)<pages-1){
																		%>
																			<a href="modiValueIndex.jsp?iStart=<%=pageDown%>"><bean:message key="str1032"/> </a>&nbsp; 
																		<%
																			}
																		%>
																		<a  href="modiValueIndex.jsp?iStart=<%=pages-1%>"><bean:message key="str1033"/></a></td>
