...
<html>
	<head>
		<title><bean:message key="str3327"/></title>
					...
					<table width=727 border=0 cellpadding=2 cellspacing=1 align=center bgcolor="#e7e7e7">
						...
						<%		out.print(tableList);
								...
								for (int i = 0; i < tableList.size(); i++) {
									...
									String idx = "", quo_name = "", cust_name = "", give_addr = "", sale_name = "", form_id = "";
						...
						<tr style="background-color: #f9f9f9;" id="changcolor_tr<%=i%>" onmouseover="MM_changeProp('changcolor_tr<%=i%>','','backgroundColor','#ffffff','DIV')" onmouseout="MM_changeProp('changcolor_tr<%=i%>','','backgroundColor','#f9f9f9','DIV')">
							...
							<td style="color: #000000;" align="center">
								<a href="addIndex.jsp?id=<%=idx%>&menu_id=<%=menu_idx%>&form_id=<%=form_id%>" target="_self"><img src=/img/frost.gif width=16 height=16 border=0 alt="退货"> </a>
							</td>
						</tr>
						<%
						}
						...
						<tr>
							...
							<td align="right" colspan="3">
								<a href="refundIndex.jsp?iStart=0"><bean:message key="str613"/> </a>&nbsp; &nbsp;
								<%
								if (Integer.parseInt(iStart) > 0) {
								%>
								<a href="refundIndex.jsp?iStart=<%=pageUp%>"><bean:message key="str1031"/></a> &nbsp;
								<%
										}
										if (Integer.parseInt(iStart) < pages - 1) {
								%>
								<a href="refundIndex.jsp?iStart=<%=pageDown%>"><bean:message key="str1032"/> </a>&nbsp;
								<%
								}
								%>
								<a href="refundIndex.jsp?iStart=<%=pages - 1%>"><bean:message key="str1033"/></a>
							</td>
						</tr>
						<%
						}
