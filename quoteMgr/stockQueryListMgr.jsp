					...
					<table width=725 border=0 cellpadding=2 cellspacing=1 align=center bgcolor="#e7e7e7">
						...
						<tr style="background-color: #f9f9f9;" id="changcolor_tr<%=i%>" onmouseover="MM_changeProp('changcolor_tr<%=i%>','','backgroundColor','#ffffff','DIV')" onmouseout="MM_changeProp('changcolor_tr<%=i%>','','backgroundColor','#f9f9f9','DIV')">
							...
							<td style="color: #000000; padding: 2px 5px;" align=center>
								<a href="quoteMgrExp.jsp?query_id=<%=query_id%>" TARGET=appwin onclick="mydefss()" title="点击进入报价单管理"><img src="/img/edit.png" border="0">
								</a>
							</td>
						</tr>
						<%
						...
						<tr>
							<td align="left" colspan="2">
								<bean:message key="str3988"/><%=counter%><bean:message key="str1028"/> &nbsp;<bean:message key="str1145"/><%=Integer.parseInt(iStart) + 1%><bean:message key="str1146"/>&nbsp;&nbsp;<bean:message key="str3988"/><%=pages%><bean:message key="str1146"/>
							</td>
							<td align="right" colspan="2">
								<a href="quoteMgr.jsp?iStart=0"><bean:message key="str1147"/> </a>&nbsp; &nbsp;
								<%
								if (Integer.parseInt(iStart) > 0) {
								%>
								<a href="quoteMgr.jsp?iStart=<%=pageUp%>"><bean:message key="str1148"/></a> &nbsp;
								<%
										}
										if (Integer.parseInt(iStart) < pages - 1) {
								%>
								<a href="quoteMgr.jsp?iStart=<%=pageDown%>"><bean:message key="str1149"/> </a>&nbsp;
								<%
								}
								%>
								<a href="quoteMgr.jsp?iStart=<%=pages - 1%>"><bean:message key="str1150"/></a>
							</td>
						</tr>
						<%
						}
