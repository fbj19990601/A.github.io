...
var _VersionInfo="清空";	//版本信息

//==================================================== WEB 页面显示部分 =====================================================
var strFrame;		//存放日历层的HTML代码
document.writeln('<iframe id=meizzDateLayer Author=wayx frameborder=0 style="position: absolute; width:144px; height: 215px; z-index: 9998; display: none" scrolling="no"></iframe>');
strFrame='<style>';
...
strFrame+='<table border=1 cellspacing=0 cellpadding=0 width=142 height=165 bordercolor=#ff9900 bgcolor=#ff9900 Author="wayx">';
...
strFrame+='        onclick="parent.meizzPrevM()" title="Move up 1 month" Author=meizz><b Author=meizz>&lt;</b>';
strFrame+='        </td><td width=60 align=center style="font-size:12px;cursor:default" Author=meizz ';
strFrame+='onmouseover="style.backgroundColor=\'#FFD700\'" onmouseout="style.backgroundColor=\'white\'" ';
strFrame+='onclick="parent.tmpSelectYearInnerHTML(this.innerText.substring(0,4))" title="Click here to select the Year"><span Author=meizz id=meizzYearHead></span></td>';
...
strFrame+='        title="Click here to select the Month"><span id=meizzMonthHead Author=meizz></span></td>';
strFrame+='        <td width=16 bgcolor=#ff9900 align=center style="font-size:12px;cursor: hand;color: #ffffff" ';
strFrame+='         onclick="parent.meizzNextM()" title="Move down 1 month" Author=meizz><b Author=meizz>&gt;</b></td></tr>';
...
strFrame+='        <td colspan=3 align=right Author=meizz><span onclick=parent.Clear() style="font-size:12px;cursor: hand"';
strFrame+='         Author=meizz title="' + _VersionInfo + '"><u>clear</u></span>&nbsp;</td></tr>';
strFrame+='    </table></td></tr><tr Author="wayx"><td Author="wayx">';
strFrame+='        <table border=0 cellspacing=1 cellpadding=0 width=100% Author="wayx" bgcolor=#FFFFFF>';
strFrame+='          <tr Author="wayx"><td Author=meizz align=left><input Author=meizz type=button class=button value="<<" title="Move up for 1 year" onclick="parent.meizzPrevY()" ';
strFrame+='             onfocus="this.blur()" style="font-size: 12px; height: 20px"><input Author=meizz class=button title="Move up for 1 month" type=button ';
...
strFrame+='             onfocus="this.blur()" title="Now" style="font-size: 12px; height: 20px; cursor:hand"></td><td ';
strFrame+='             Author=meizz align=right><input Author=meizz type=button class=button value=" >" onclick="parent.meizzNextM()" ';
strFrame+='             onfocus="this.blur()" title="Move down for 1 month" class=button style="font-size: 12px; height: 20px"><input ';
strFrame+='             Author=meizz type=button class=button value=">>" title="Move down 1 year" onclick="parent.meizzNextY()"';
...
function setday(tt,obj) //主调函数
{
	...
	dads.display = '';
	document.all.meizzDateLayer.focus();//使页面自动跟控件显示到最下面
...
{
  if (strYear.match(/\D/)!=null){alert("This is not number！");return;}
  var m = (strYear) ? strYear : new Date().getFullYear();
  if (m < 1000 || m > 9999) {alert("Pelase enter between 1000 to  9999 ！");return;}
  var n = m - 50;
  if (n < 1000) n = 1000;
...
{
  if (strMonth.match(/\D/)!=null){alert("This is not number！");return;}
...
function meizzDayClick(n,ex)  //点击显示框选取日期，主输入函数*************
{
  ...
  if (mm < 10){mm =  mm;}//mm = "0" + mm; 改成没有前导数，因为在判断日期大小时，有前导数无法判断。下同
  if (outObject)
  {
