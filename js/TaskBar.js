...
Ext.extend(Ext.ux.TaskBar, Ext.util.Observable, {
    init : function(){
		this.startMenu = new Ext.ux.StartMenu({
			...
			height: 300,
			shadow: true,
			title: document.getElementById("user_name").value, //get_cookie('memberName'),
			width: 300
		});
		...
		this.startMenu.addTool({
			text:'Settings',
			iconCls:'settings',
			handler:function(){ window.location = "/main.jsp?menu_id=2EpTpGfO71V2R8KVx8S&menu_class=2&subsys_code=CTR"; },
			scope:this
		},'-',{
			text:'Logout',
			iconCls:'logout',
			handler:function(){ window.location = "/out.jsp"; },
			scope:this
		});
		...
		this.startBtn = new Ext.Button({
            ...
            template: new Ext.Template(
				...
				'<td class="ux-startbutton-left"><i>&#160;</i></td><td class="ux-startbutton-center"><em unselectable="on"><button class="x-btn-text" type="{1}" style="height:86px;">{0}</button></em></td><td class="ux-startbutton-right"><i>&#160;</i></td>',
				"</tr></tbody></table>")
        });
...
Ext.ux.TaskBar.TaskButton = function(win, el){
    ...
    Ext.ux.TaskBar.TaskButton.superclass.constructor.call(this, {
        ...
        template: new Ext.Template(
			'<table border="0" cellpadding="0" cellspacing="0" class="x-btn-wrap"><tbody><tr>',
			'<td class="ux-taskbutton-left"><i>&#160;</i></td><td class="ux-taskbutton-center"><em unselectable="on"><button class="x-btn-text" type="{1}" style="height:86px;">{0}</button></em></td><td class="ux-taskbutton-right"><i>&#160;</i></td>',
			"</tr></tbody></table>")
    });
};
