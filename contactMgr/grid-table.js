   ...
   function createCustomer(cust_id,class_id){
	...
	var data=Math.round(Math.random()*10000);
	 ...
	 'group_data.jsp?cust_id='+cust_id+"&class_id="+class_id+"&group_type=1&datas="+data,{
	  method:'get',
...
function addCustomer(){
	var count= document.getElementById('join').value;
	if(count==0){
		Ext.MessageBox.alert('XSaas 提示','没有要添加的联系人记录');
		return;
	  }else{
		     var res=-1;
		     for(var i=0;i<count;i++){
		    ...
		    if(res>-1){
		      ...
		      Ext.MessageBox.alert('XSaas 提示','请选中要添加的联系人记录');
		    }
	    }
}
//删除联系人操作
function delCustomer(){
	var count= document.getElementById('count').value;
	if(count==0){
		Ext.MessageBox.alert('XSaas 提示','没有要删除的联系人记录');
		return;
	  }else{
		     var res=-1;
		     for(var i=0;i<count;i++){
