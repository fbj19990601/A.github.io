...
<html>
  ...
  function chechIfo()
  {
	   if(confirm('Are you sure to write off the customer？')) 
	{
		return true;
	}
	else
			...
			<%
			    if (custArray != null && custArray.size() > 0) {
					...
					<tr style="background-color:#f9f9f9; " id="changcolor_tr<%=i%>" onMouseOver="MM_changeProp('changcolor_tr<%=i%>','','backgroundColor','#ffffff','DIV')"	onMouseOut="MM_changeProp('changcolor_tr<%=i%>','','backgroundColor','#f9f9f9','DIV')">
						<td style=" color:#000000;" align="center"><a href="/customerMgr/Custinfo.jsp?obj_cust_id=<%=cust_id%>&user_id=" TARGET=appwin onclick="mydefss()"><%=cust_name%></a></td>
						...
						<td style=" color:#000000;" align=center><a href="/doTradeReg.do?cust_id=<%=cust_id%>&cust_state=2&trade_type_code=0167" target="_self" onClick="return chechIfo()"><img src=/img/del.gif width=16	height=16 border=0 alt="write off"></a></td>
					</tr>
					<%
					i++;
					}
					...
					<tr>
						...
						<td align="right" colspan="2"><a href="cancelIndex.jsp?iStart=1&meun_id=<%=meun_idx%>"><bean:message key="str613"/> </a>&nbsp;&nbsp;<a href="cancelIndex.jsp?iStart=<%=pageUp%>&meun_id=<%=meun_idx%>"><bean:message key="str614"/></a>	&nbsp;<a href="cancelIndex.jsp?iStart=<%=pageDown%>&meun_id=<%=meun_idx%>"><bean:message key="str615"/></a>&nbsp;<a href="cancelIndex.jsp?iStart=<%=pages%>&meun_id=<%=meun_idx%>"><bean:message key="str616"/></a></td>
					</tr>
					<%
			}
			%>
