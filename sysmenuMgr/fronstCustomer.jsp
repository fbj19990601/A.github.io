...
<html>
  ...
  function chechIfo()
  {
	   if(confirm('Are you sure to freeze the member？')) 
	{
		return true;
	}
	else
			...
			<%
			    if (custArray != null && custArray.size() > 0) {
					...
					<tr style="background-color:#f9f9f9; " id="changcolor_tr<%=i%>" onMouseOver="MM_changeProp('changcolor_tr<%=i%>','','backgroundColor','#ffffff','DIV')"	onMouseOut="MM_changeProp('changcolor_tr<%=i%>','','backgroundColor','#f9f9f9','DIV')">
						...
						<td style=" color:#000000;" align=center><a href="/doTradeReg.do?cust_id=<%=cust_id%>&cust_state=1&trade_type_code=0167" target="_self" onClick="return chechIfo()"><img src=/img/frost.gif width=16	height=16 border=0 alt=" freeze the member"></a></td>
					</tr>
					<%
					i++;
					}
					...
					<tr>
						...
						<td align="right" colspan="2"><a href="fronstIndex.jsp?iStart=0&meun_id=<%=meun_idx%>"><bean:message key="str613"/></a>&nbsp;&nbsp;<a href="fronstIndex.jsp?iStart=<%=pageUp%>&meun_id=<%=meun_idx%>"><bean:message key="str614"/></a>	&nbsp;<a href="fronstIndex.jsp?iStart=<%=pageDown%>&meun_id=<%=meun_idx%>"><bean:message key="str615"/></a>&nbsp;<a href="fronstIndex.jsp?iStart=<%=pages%>&meun_id=<%=meun_idx%>"><bean:message key="str616"/></a></td>
					</tr>
					<%
			}
			%>
